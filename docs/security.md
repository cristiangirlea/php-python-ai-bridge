...
- Redaction is assistive, not a compliance control. The rules find only the structured identifiers they recognise, and the optional named-entity model was trained on English news, so its recall on names from other domains or languages is lower and unmeasured here; review redacted output before treating it as free of personal data. The model files are a third-party ONNX conversion of the original weights, pinned by revision and hash but not signed by the original author.
